 ...
 if(isset($_POST['go']) || isset($_SESSION['course']))
  {
	...
	$test=mysql_fetch_array(mysql_query("select test_id,equal_weight,max_marks,neg_marking from tests where active=1 and course_id='$_SESSION[course]' "));
	
	$eligible=mysql_fetch_array(mysql_query("select done from exam_taken where rollnumber='$_SESSION[rollnumber]' and test_id='$test[test_id]' and done=1 "));
